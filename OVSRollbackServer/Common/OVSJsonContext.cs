...
namespace OVS.Rollback.Common
{
    ...
    /// writes (config file, matchmaker HTTP payloads, diagnostic logging). Native AOT trims
    /// the reflection-based serializer, so all (de)serialization must go through a
